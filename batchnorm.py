...
from models.nn import NeuralNet, DenseLayer, SigmoidActivation, Batchnorm, WeightUpdater
...
def get_model(num_hidden_layers, num_hidden_nodes):
    layers = [
        DenseLayer(num_hidden_nodes),
        Batchnorm(),
        SigmoidActivation(),
    ]
    for _ in range(num_hidden_layers - 1):
        layers.append(DenseLayer(num_hidden_nodes))
        layers.append(SigmoidActivation())
    layers.append(DenseLayer(1))
    layers.append(SigmoidActivation())
    model = NeuralNet(
        layers, 200, True, batch_size=32, weights_updater=WeightUpdater(0.1)
    )
    return model
